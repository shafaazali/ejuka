...
<div class="site-section bg-light" id="contact-section">
   ...
   $sq=" select e.Q_cid_fk,SUM(e.score) result,category.C_Name   from  exam  e,category where e.E_user_fk=$id and e.Q_cid_fk=category.C_ID group by e.Q_cid_fk order by result desc  limit 1";
   ...
   $cid=$row['Q_cid_fk'];
    ?>
        <div class="row">
          ...
          <h2> Yor Exam is over, Based on your answer you can choose the options listed below </h2>
          <div class="col-12 mb-3">
            <h2 class="text-black"><?php echo $category ;?></h2>
          </div>
          <ul>
          <?php 
           $q=" SELECT * FROM category_result where C_id_fk='$cid'";
           $res=mysqli_query($link,$q);
           while($row=mysqli_fetch_assoc($res))
           {
            echo "<li>".$row['Result']."</li>";
           }
          ...
          <div class="col-12 mb-3">
            <a href='request.php'><input type="button" class="btn btn-block btn-primary" value="Click here free councling "></a>
          </div>
        </div>
      </div>
    </div>
